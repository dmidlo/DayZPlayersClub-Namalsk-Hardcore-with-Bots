	...
	override void StartingEquipSetup( PlayerBase player, bool clothesChosen )
	{
		...
		if ( itemClothing )
		{
			...
			if ( throwDice == 0 )
				...
				itemEnt = itemClothing.GetInventory().CreateInInventory( "Chemlight_Green" );
			else if ( throwDice == 3 )
				itemEnt = itemClothing.GetInventory().CreateInInventory( "Chemlight_Blue" );
			else
				itemEnt = itemClothing.GetInventory().CreateInInventory( "Chemlight_Yellow" );
			player.SetQuickBarEntityShortcut( itemEnt, 2 );

			itemEnt = itemClothing.GetInventory().CreateInInventory( "ItemPDA" );
			SetRandomHealth( itemEnt );
		}
