class UpgradeGravitiToLvl3 extends RecipeBase
{	
	override void Init()
	{   
		m_Name = "Upgrade"; //Название-действие
		...
		InsertIngredient(0,"ONLI_Graviti2");      //Вы можете вставить несколько ингредиентов таким образом
		...
		m_IngredientDestroy[0] = true;//true = разобрать, false = ничего не делать (удаляется индегриент после крафта.)
		m_IngredientUseSoftSkills[0] = false;// установите значение "true", чтобы разрешить изменение значений softskills для этого ингредиента
		
		//Ингредиенты #2
		//ИСПОЛЬЗУЕМЫЙ ПРЕДМЕТ
		InsertIngredient(1,"dzn_lehs_battery");
		...
		//РЕЗУЛЬТАТ 
		AddResult("ONLI_Graviti3"); //Добавить результаты здесь
